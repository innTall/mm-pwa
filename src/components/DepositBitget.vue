<script setup>
import FooterBitget from './FooterBitget.vue';
import { watch } from 'vue';
import { useDepoBitgetStore } from '@/stores/depoBitget.js';
import { storeToRefs } from "pinia";

const { addBlock, deactivateBlock, updateLastBalance } = useDepoBitgetStore();
const { blocks, balances, lastBalance } = storeToRefs(useDepoBitgetStore());
...
<template>
	<div class="">
		...
		<div class="fixed bottom-0 left-0 w-full h-12 z-10 bg-gray-900">
			...
			<FooterBitget :lastBalance="lastBalance" />
		</div>
	</div>
</template>
<style scoped></style>
